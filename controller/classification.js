...
class Classification {
   ...
   train(documents = [], classes = [], names = []) {
      log(chalk.green("Training documents: "));
   ...
   test(documents = [], k = 1) {
      log(chalk.green("Testing documents: "));
      ...
      this.tests.TFIDF.map((query, index) => {
         ...
         log(chalk.green("predicting document: ") + index);
   ...
   knn(k = 1, query = []) {
      ...
      log(chalk.yellow("prediction: "), prediction);
   ...
   accuracy(predictions = [], actuals = []) {
      log("");
      log(chalk.green("Predictions class:\n") + predictions.join(", "));
      log(chalk.green("Actual class:\n") + actuals.join(", "));
      ...
      log(chalk.magenta("Accuracy: ") + accuracy);
